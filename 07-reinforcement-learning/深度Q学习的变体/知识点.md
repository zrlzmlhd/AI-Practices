...
深度Q学习的变体/
├── core/               # 核心配置和数据结构
│   ├── config.py       # DQNVariantConfig 超参数配置
│   ├── enums.py        # DQNVariant 算法枚举
│   └── types.py        # Transition 数据类型
├── buffers/            # 经验回放缓冲区
│   ├── base.py         # ReplayBuffer (均匀采样)
│   ├── sum_tree.py     # SumTree 数据结构
│   ├── prioritized.py  # PrioritizedReplayBuffer
│   └── n_step.py       # NStepReplayBuffer
├── networks/           # 神经网络架构
│   ├── base.py         # DQNNetwork 标准Q网络
│   ├── dueling.py      # DuelingNetwork
│   ├── noisy.py        # NoisyLinear, NoisyNetwork
│   ├── categorical.py  # CategoricalNetwork (C51)
│   └── rainbow.py      # RainbowNetwork
├── agents/             # 智能体实现
│   └── variant_agent.py # DQNVariantAgent 统一接口
├── utils/              # 工具函数
│   ├── training.py     # 训练循环
│   ├── comparison.py   # 变体对比
...
from utils import train_agent, evaluate_agent
...
# 评估
mean_reward, std = evaluate_agent(agent, "CartPole-v1")
print(f"Performance: {mean_reward:.1f} ± {std:.1f}")
```
